...
from model import *
from loss import *
from utils import tf_colorize

# input path
tf.app.flags.DEFINE_string('dense_folder', None,
                           """Root path to dense folder.""")
tf.app.flags.DEFINE_string('dtu_data_root', None,
                           """Root directory for DTU dataset""")
tf.app.flags.DEFINE_string('dense_folder_list', None,
                           """Text file for list of dense folders""")
tf.app.flags.DEFINE_string('output_folder', 'output',
                           """Output folder""")
...
def mvsnet_pipeline(mvs_list, output_folder=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(len(mvs_list)):
            ...
            try:
                out_init_depth_map, out_vis_depth_map, out_prob_map, out_vis_prob_map, out_images, out_cams, out_index = sess.run(
                    [init_depth_map, vis_depth_map, prob_map, vis_prob_map, scaled_images, scaled_cams, image_index])
            ...
            out_vis_depth_image = np.squeeze(out_vis_depth_map)
            out_vis_depth_image = cv2.cvtColor(out_vis_depth_image * 255.0, cv2.COLOR_BGR2RGB)
            out_prob_map = np.squeeze(out_prob_map)
            out_vis_prob_map = np.squeeze(out_vis_prob_map)
            out_vis_prob_map = cv2.cvtColor(out_vis_prob_map * 255.0, cv2.COLOR_BGR2RGB)
            ...
            cv2.imwrite(vis_depth_map_path, out_vis_depth_image)
            write_pfm(prob_map_path, out_prob_map)
            cv2.imwrite(vis_prob_map_path, out_vis_prob_map)
...
def main(_):  # pylint: disable=unused-argument
    ...
    if FLAGS.dense_folder is not None:
